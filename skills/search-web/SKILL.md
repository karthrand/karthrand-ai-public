...
用最小搜索链路整合 `Context7`、`Exa`、`TinyFish`、网页正文读取、`mcp-deepwiki` 和 `github-fetcher`。普通搜索直接使用当前会话已有工具；环境缺失时提示用户手动运行 `$search-web setup` 并重启 code agent。
...
1. 如果用户参数为 `setup`，读 `references/setup.md`，执行初始化流程，然后停止当前搜索。
2. 普通搜索不运行 `bash scripts/detect.sh`，直接按搜索目标选择工具。
3. 配置项类问题（阈值、开关、flag、option、parameter、env）先读 `references/strategies/config-index-shortcut.md`。
4. 技术文档先走 `Context7`（先 `resolve-library-id` 再 `query-docs`）。
5. 联网搜索先使用 `Exa`；当 `Exa` 失败、额度到限、服务不可用或空结果时，按 `references/tinyfish-fallback.md` 使用 `TinyFish` CLI 备用。
6. 搜索命中 GitHub 仓库时走 `mcp-deepwiki`；需读具体文件或目录时走 `github-fetcher`。
7. 只有用户已给出明确 URL、`Exa` 已定位 URL 或 `TinyFish` 已定位 URL 后才允许网页正文读取。
8. 最终按 `references/rules/output-format.md` 组织回答。
...
- 普通搜索中，`Context7`、`Exa`、`mcp-deepwiki`、`github-fetcher` 报未注册、未加载、连接失败或 `server unavailable` 时，停止当前搜索，提示用户运行 `$search-web setup` 并重启 code agent。
- 普通搜索不自动执行 `setup-mcp.sh`，不自动写入任何 MCP 配置。
- `TinyFish` 是 CLI 备用工具；只有 `Exa` 已调用但失败、额度到限、服务不可用或空结果时才尝试使用。
- 宿主类型未识别时，普通搜索照常进行；用户可主动运行 `$search-web setup` 获取通用 MCP JSON。
...
- MCP 未加载时反复重试（应提示用户运行 `$search-web setup` 并重启）

## 按需继续加载

只有在以下场景再读 `examples/usage.md`：

- 需要 `Context7`、`Exa`、`TinyFish`、网页正文读取、`mcp-deepwiki` 或 `github-fetcher` 的参数示例
...
只有用户调用 `$search-web setup`，或用户明确要求配置/修复搜索环境时，读 `references/setup.md`。
...
- 明确标注来源类型：`Context7`、`Exa`、`TinyFish`、网页正文读取、`mcp-deepwiki`、`github-fetcher`
- 保留原始链接、文档入口或仓库标识
